...
Tracker Position は **Experimental** です。特に Head や直接 Bone に割り当てる場合は、当面 Inspector の Apply Position を OFF にし、Rotation のみを使うことを推奨します。Head の raw position は Head Bone の local position ではなく、VRChat の tracking-space alignment 用データです。tracking-space calibration なしでは、意図した Bone 位置にならない場合があります。既存の Position 機能は残っています。
