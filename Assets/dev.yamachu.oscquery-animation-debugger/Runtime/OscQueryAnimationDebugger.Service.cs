...
public partial class OscQueryAnimationDebugger
{
    private void TryStartOscQueryService()
    {
        ...
        try
        {
            ...
            Debug.Log($"[OSCQuery Bridge] サービス起動成功: {serviceName} (IP:{localIP} / TCP:{tcpPort} / UDP:{oscPort})");
        ...
        {
            Debug.LogError($"[OSCQuery Bridge] サービスの起動に失敗しました: {e.Message}, IP:{localIP} / TCP:{tcpPort} / UDP:{oscPort}");
    ...
    private static IPAddress GetLocalIPAddress()
    {
        try
        {
            ...
            // 最後のフォールバック
            Debug.LogWarning("[OSCQuery Bridge] ローカル IP アドレスを取得できません。localhost を使用します。");
            return IPAddress.Loopback;
        }
        catch (Exception e)
        {
            Debug.LogError($"[OSCQuery Bridge] IP アドレス取得エラー: {e.Message}");
            return IPAddress.Loopback;
        }
    }
    ...
    private void RefreshDiscoveredServices()
    {
        ...
        {
            Debug.LogWarning($"[OSCQuery Bridge] サービス探索の更新に失敗しました: {e.Message}");
        }
    }
    ...
    private void RegisterRemoteOscEndpoint(OSCQueryServiceProfile service)
    {
        ...
        try
        {
            ...
            Debug.Log($"[OSCQuery Bridge] 送信先として登録: {service.name} ({endpoint})");
        }
        catch (Exception ex)
        {
            Debug.LogWarning($"[OSCQuery Bridge] 送信先の登録に失敗: {service.name} - {ex.Message}");
        }
    }
    ...
    private void LogDiscoveredService(string serviceType, OSCQueryServiceProfile service)
    {
        ...
        Debug.Log($"[OSCQuery Bridge] {serviceType}サービスを発見: {service.name} ({service.address}:{service.port})");
    }
}
